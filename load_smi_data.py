"""
load_smi_data.py — Scarica SMI (Swiss Market Index, INSTRUMENT_IDX_EUROPE_E_SWMI)
...
di DAX/FTSE100/GOLD/ITALY40 — timeframe="30m", source="dukascopy").

Costante strumento confermata tramite discover_dukascopy_smi_v2.py
(non indovinata): INSTRUMENT_IDX_EUROPE_E_SWMI = 'E_SWMI'. Pattern di
naming coerente con E_DAAX (DAX) ed E_Futsee-100 (FTSE100), già in uso
nel progetto.

Idempotente: il file SQL generato cancella prima ogni riga con
symbol='SMI' esistente, poi ricarica.

Copiato da load_italy40_data.py (stessa struttura, solo strumento e
symbol cambiati) — nessuna logica nuova, minimizza rischio di errori.
"""
...
from dukascopy_python.instruments import INSTRUMENT_IDX_EUROPE_E_SWMI
...
def main():
    ...
    with open("insert_smi.sql", "w") as f:
    ...
    print(f"File insert_smi.sql generato con {len(all_rows)} righe "
