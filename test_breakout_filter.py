"""
test_breakout_filter.py — TEST 2/6 del programma di validazione (20/07/2026).

Domanda: evitare l'apertura di posizioni quando il breakout supera una
soglia (DAX >30pt, FTSE100 >15pt — il confine del bucket peggiore
osservato nel dataset di ricerca, dove R medio era sceso a 0,16/-0,02
rispettivamente) migliora davvero il risultato, o e' rumore/artefatto
come persistenza e filtro orario?

Stesso protocollo in un solo script (motore -> sanity check -> run 5
periodi -> bootstrap), stessa struttura di test_hour_filter.py.
...
CAPITAL0 = 2000.0
SYMBOLS = ["DAX", "FTSE100"]
MAX_BREAKOUT_DISTANCE = {"DAX": 30.0, "FTSE100": 15.0}
N_BOOT = 2000
SEED = 20260721
...
# =====================================================================
# 1) MOTORE — sottoclasse isolata, filtro distanza breakout
# =====================================================================

class BacktestEngineBreakoutDistanceFilter(BacktestEngineFloatingKillSwitch):
    def __init__(self, *args, max_breakout_distance: dict[str, float] = None, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_breakout_distance = max_breakout_distance or {}
    ...
    def run(self, data: dict[str, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for ts in all_timestamps:
            ...
            for name in tradable_instruments:
                ...
                # --- NUOVO: filtro distanza breakout eccessiva ---
                max_dist = self.max_breakout_distance.get(name)
                if max_dist is not None:
                    if prev_bar["signal"] == "long":
                        breakout_level = prev_bar["rolling_high"]
                        distance = prev_bar["close"] - breakout_level
                    else:
                        breakout_level = prev_bar["rolling_low"]
                        distance = breakout_level - prev_bar["close"]
                    if pd.isna(distance) or distance > max_dist:
                        continue
                # --- fine nuovo ---
...
# =====================================================================
# 2) BOOTSTRAP a blocchi di giornata (stesso protocollo di GOLD/Test 1)
# =====================================================================
...
def main():
    ...
    # --- SANITY CHECK: max_breakout_distance={} deve dare risultati identici al motore standard ---
    print("=== SANITY CHECK (max_breakout_distance={} deve dare risultati identici al motore standard) ===")
    ...
    variant_check = BacktestEngineBreakoutDistanceFilter(
        capital0=CAPITAL0, instruments=eng.INSTRUMENTS, max_breakout_distance={})
    ...
    # --- RUN sui 5 periodi ufficiali: baseline vs variante (evita breakout eccessivo) ---
    print("=== RUN sui 5 periodi ufficiali ===")
    print(f"Soglie: {MAX_BREAKOUT_DISTANCE}\n")
    ...
    for label, start_str, end_str in PERIODS:
        ...
        eng_variant = BacktestEngineBreakoutDistanceFilter(
            capital0=CAPITAL0, instruments=eng.INSTRUMENTS, max_breakout_distance=MAX_BREAKOUT_DISTANCE)
    ...
    print(f"Trade totali: baseline={total_trades_base}  variante={total_trades_variant} "
          f"({total_trades_base - total_trades_variant} trade evitati per breakout eccessivo)")
    ...
    print(f"\nVERDETTO: se il delta osservato cade dentro l'IC 95% del rumore, o se >10-15% delle "
          f"iterazioni danno delta<=0, il filtro breakout NON e' chiaramente distinguibile dal rumore "
          f"campionario (Protocollo Anti-Rumore, principio 4) — stesso standard usato per GOLD/Test 1.")

    summary_df = pd.DataFrame(rows)
    os.makedirs("results", exist_ok=True)
    summary_df.to_csv("results/test_breakout_filter.csv", index=False)
    print("\nDettaglio per periodo salvato in results/test_breakout_filter.csv")
