"""
extra_slot_test.py — Versione generica di extra_slot_30day_test.py:
numero di giorni feriali della finestra selezionabile al lancio
(default 30, tetto rigido 180 — oltre viene troncato con warning).

Uso:
  python extra_slot_test.py                 # default 30 giorni
  python extra_slot_test.py --days 60        # 60 giorni
  python extra_slot_test.py --days 999       # troncato a 180 con warning

Da GitHub Actions: il workflow passa --days dal valore scelto
nell'input del workflow_dispatch (vedi extra_slot_test.yml).

Tutto il resto invariato rispetto a extra_slot_30day_test.py: stesso
motore esteso (BacktestEngineExtendedOrders, invariato), stesse
metriche di periodo (win/loss, PF, PnL, drawdown, R medio) per
baseline/extra/combinato, stesso fix sul filtro temporale degli skip
(extra_slot_skip_pnl_log / extra_slot_skip_minsize_log).
...
Nessuna modifica al motore standard. Nessuna scrittura su D1.
"""
...
def parse_args() -> int:
    parser = argparse.ArgumentParser(description="Analisi slot extra 4/5 su N giorni feriali (tetto 180)")
...
def main():
    ...
    extra_window.to_csv(f"extra_slot_{suffix}_trades.csv", index=False)
    summary_df.to_csv(f"extra_slot_{suffix}_summary.csv", index=False)

    print(f"\nCompletato. File: extra_slot_{suffix}_trades.csv, extra_slot_{suffix}_summary.csv")
